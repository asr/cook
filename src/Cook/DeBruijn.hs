...
-- Substitution needs to adjust the inserted expression so that free variables
-- refer to the correct binders.
...
subst o s v@(DVar i) | i == o    = adjust 0 s
                     ...
                     | otherwise = v
  where
    adjust n e@(DVar j) | j >= n    = DVar (j + o)
                        | otherwise = e
...
-- Convert to De Bruijn indices.  Do this by keeping a list of the bound variables
-- so that depth can be found of all variables.  Do not touch free variables.
...
-- Convert back from De Bruijn to the LC type.

fromDB :: DB -> LC IdInt
fromDB = from firstBoundId
  where
    from (IdInt n) (DVar i) | i < 0     = Var (IdInt i)
