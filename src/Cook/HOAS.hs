...
-- need to represent variables for free variables and also during conversion.

data HOAS = HVar IdInt | HLam (HOAS -> HOAS) | HApp HOAS HOAS

-- To compute the normal form, first convert to HOAS, compute, and convert
-- back.

nf :: LC IdInt -> LC IdInt
nf = toLC . nfh . fromLC

-- The substitution step for HOAS is simply a Haskell application since we use
-- a Haskell function to represent the abstraction.
...
-- Convert to higher order abstract syntax.  Do this by keeping a mapping of
-- the bound variables and translating them as they are encountered.

fromLC :: LC IdInt -> HOAS
fromLC = from M.empty
  where
    from m (Var v)   = maybe (HVar v) id (M.lookup v m)
    from m (Lam v e) = HLam $ \x -> from (M.insert v x m) e
    from m (App f a) = HApp (from m f) (from m a)
    
-- Convert back from higher order abstract syntax.  Do this by inventing a new
-- variable at each λ-abstraction.
