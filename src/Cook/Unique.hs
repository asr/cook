...
-- Normalisation is run in a State monad with the next free variable.
...
-- Substitution proceeds by cloning the term that is inserted at every place it
-- is put.
-- (TODO: No need to clone λ-free terms.)
...
-- Do the actual translation of the term to unique variables.  We keep a mapping
-- of old variable names to new variable names.  Free variables are just left alone
-- since they are already uniquely named.
